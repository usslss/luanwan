...
//统计数据库中已经存在的option选项
$sqlShow = "SELECT DISTINCT slider_show FROM slider ";
$sqlfinish2 = mysqli_query($link,$sqlShow);

while($row=mysqli_fetch_array($sqlfinish2)){
    $existoptionArr[]=$row["slider_show"];
}
...
for($i=0;$i<$optionSum;$i++){
    ...
    //设置已选择项
    if (($optionArr[$i]["selectValue"])==$slider_show){
...
$(document).ready(function(){
    ...
    var strrr='<?php echo $slider_summary?>';
...
<div class="x-body">
 <form action="slider_edit_finish.php" method="post" enctype="multipart/form-data">
                
                <table class="layui-table" >
                    <tbody >
                    <input type="hidden" value="<?php echo $slider_id;?>" name="slider_id"></input>
                    <tr>
                            <th width="120" colspan="1">轮播名称</th>
                            <td colspan="1" ><div class="layui-input-inline">
                  
                  <input type="text" style="width:300px" name="slider_name" required="" lay-verify="required"
                  autocomplete="off" class="layui-input" value="<?php echo $slider_name;?>">
              </div>
            			  <td colspan="1">显示顺序</td>
            			  <td colspan="1">
<div class="layui-form">
<div class="layui-inline" style="width:100px">            			  
<select name="slider_show" lay-verify=""">
